...
4. Create an `.env` and fill in your values
   ...
   | `INVERTER_CHARGE_DISCHARGE_EFFICIENCY` | Efficiency of charging and discharging the battery in percent. Example: `10` means that only 10% of the input energy is effectively stored.                                                                                                                                                                                   | `90`                         | A number between `0` and `100`, typically between `80` and `100`                                                             |
   ...
   | `GRID_FEE_WINDOWS`                     | This variable CAN be set to account for time-variable grid fees (module 3 of § 14a EnWG). The values are the difference to the standard tariff, which the price reported by Tibber already contains. The grid operator publishes the windows in its price list for a whole calendar year, so this has to be updated annually. | -                            | A comma separated list of `<START>-<END>=<DIFFERENCE_IN_CENTS>`. Example: `10:00-14:00=-5.40,17:00-22:00=4.15`               |
   | `INFLUXDB_URL`                         | The URL of the InfluxDB to write to. When running via `compose.yaml` the app container sets this to reach the database container.                                                                                                                                                                                             | -                            | A URL. Example: `http://influxdb:8086`                                                                                       |
   ...
   A variable set in the actual environment (e.g. via `environment:` in the `compose.yaml`) wins over both files.
