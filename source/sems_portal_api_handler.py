...
class SemsPortalApiHandler(LoggerMixin):
    ...
    def get_power_usage_in_timeframe_in_watt_hours(
        ...
        self.log.debug(
            f"Getting estimated power usage between {timestamp_start} and {timestamp_end}"
        )
        ...
        power_usage_of_today_in_watt_hours = (
            self.get_average_power_consumption_per_day()
        )
        self.log.debug(
            f"Expected power usage of the day is {power_usage_of_today_in_watt_hours} Wh"
        )
        power_usage_of_today_in_watt_seconds = (
            power_usage_of_today_in_watt_hours * 60 * 60
        )

        average_power_usage_in_watts = power_usage_of_today_in_watt_seconds / (
            60 * 60 * 24
        )
        self.log.debug(
            f"Average power consumption today is {average_power_usage_in_watts} W"
        )

        power_usage_during_the_day_in_watt_seconds = (
            average_power_usage_in_watts
            * day_duration.total_seconds()
            * 2
            * factor_power_usage_during_the_day
        )
        power_usage_during_the_night_in_watt_seconds = (
            average_power_usage_in_watts
            * night_duration.total_seconds()
            * 2
            * factor_power_usage_during_the_night
        )
        power_usage_during_the_day_in_watt_hours = int(
            power_usage_during_the_day_in_watt_seconds / (60 * 60)
        )
        power_usage_during_the_night_in_watt_hours = int(
            power_usage_during_the_night_in_watt_seconds / (60 * 60)
        )
        self.log.info(
        ...
        return (
            power_usage_during_the_day_in_watt_hours
            + power_usage_during_the_night_in_watt_hours
        )
    ...
    def calculate_day_night_duration(
        ...
        while current_time < timestamp_end:
            ...
            day_start_time = datetime.combine(
                current_time.date(), day_start, tzinfo=self.timezone
            )
            night_start_time = datetime.combine(
                current_time.date(), night_start, tzinfo=self.timezone
            )
            ...
            if current_time < day_start_time or current_time >= night_start_time:
                ...
                self.log.trace(
                    f"Adding {slot_duration} to the night duration (now: {duration_night}"
                )
                ...
                self.log.trace(
                    f"Adding {slot_duration} to the day duration (now: {duration_day}"
                )
